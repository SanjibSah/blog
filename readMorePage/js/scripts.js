...
fetch(`https://admin.nextjavascript.com/fake-api/posts/${userId}`)
  .then((res) => res.json())
  .then((post) => {
    const { title, author, pulished_date, featured_image, content } = post;
    const readMoreHtml = `
            ...
            <figure class="mb-4"><img class="img-fluid rounded" src="${featured_image}" alt="${title}" width="900" height="400"/></figure>
